    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2">
        <div className="relative w-full max-w-sm rounded bg-white dark:bg-gray-800 p-4 shadow-lg">
          <button onClick={() => setShowEntrega(false)} className="absolute top-2 right-2 text-gray-500 hover:text-black dark:hover:text-white">✖</button>
          <h3 className="text-lg font-semibold text-green-700 dark:text-green-400 mb-3">Entregar atividade</h3>
          <form onSubmit={enviar}>
            <label className="block text-sm mb-1 text-green-700 dark:text-green-400">Aula</label>
            <select value={selAula} onChange={(e) => setSelAula(e.target.value)} className="w-full mb-3 rounded border px-2 py-1 dark:bg-gray-700 dark:text-white" required>
            ...
            <input type="file" onChange={(e) => setSelArquivo(e.target.files[0] || null)} className="mb-4 w-full text-xs" required />
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setShowEntrega(false)} className="px-3 py-1 rounded bg-gray-300 dark:bg-gray-600">Cancelar</button>
              <button type="submit" className="px-4 py-1 rounded bg-green-600 text-white">Enviar</button>
            </div>
          </form>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 p-4 sm:p-6">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-green-700 dark:text-green-400">Minhas Aulas</h1>
        <button onClick={() => setShowEntrega(true)} className="flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Entregar atividade</button>
      </div>
      <div className="mb-6 relative">
        <input type="text" value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar aula..." className="w-full p-2 pl-10 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-black dark:text-white" />
